...
module.exports = {
    ...
    searchflight: (currdate, callback) => {
        pool.query(
            'select a.logo,a.airlinename,f.flightnumber,t.source,t.destination,t.schdate,t.est_arrival_time,t.depature_time,t.firstclass,t.economyclass,t.businessclass, t.schid, f.focc - COALESCE(SUM(CASE WHEN b.seattype = \'f\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END), 0) AS frem,   f.bocc - COALESCE(SUM(CASE WHEN b.seattype = \'b\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END), 0) AS brem, f.eocc-COALESCE(SUM(CASE WHEN b.seattype=\'e\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END),0) as erem from airline a join flight f on f.aid=a.aid join travelschedule t on t.fid=f.fid left join booking b on b.schid=t.schid where schdate>=? and t.status=\'active\' group by t.schid, f.focc, f.bocc,f.eocc order by schdate asc;',
            [
                currdate
            ],
            (error, results, fields) => {
        ...
        try {
            ...
            for (const row of rows) {
                ...
                const [emailResults] = await connection.query(
                    'SELECT a.logo,b.bid,u.email,a.airlinename,f.flightnumber,t.source,t.destination,b.booked_seats,t.schdate,b.dateofbooking FROM user u JOIN booking b ON u.uid=b.uid join travelschedule t on b.schid=t.schid join flight f on f.fid=t.fid join airline a on a.aid=f.aid WHERE b.schid=?',
                    [row.schid]
                );
    ...
    cancelsch: async (schid, callback) => {
        // pool.getConnection((error, connection) => {
        //     if (error) {
        //         connection.release();
        //         return callback(error);
        ...
        try {
            ...
            await connection.query('update travelschedule set status=\'cancelled\' where schid=?', [schid]);


            await connection.query('UPDATE booking SET status=\'cancelled\' WHERE schid=?', [schid]);
    ...
    searchbyall: (date, source, destination, callback) => {
        pool.query(
            'select a.logo,a.airlinename,f.flightnumber,t.source,t.destination,t.schdate,t.est_arrival_time,t.depature_time,t.firstclass,t.economyclass,t.businessclass, t.schid,  t.schid, f.focc - COALESCE(SUM(CASE WHEN b.seattype = \'f\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END), 0) AS frem,   f.bocc - COALESCE(SUM(CASE WHEN b.seattype = \'b\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END), 0) AS brem, f.eocc-COALESCE(SUM(CASE WHEN b.seattype=\'e\' and b.status=\'active\' THEN b.booked_seats ELSE 0 END),0) as erem from airline a join flight f on f.aid=a.aid join travelschedule t on t.fid=f.fid left join booking b on b.schid=t.schid where lower(source)=? and lower(destination)=? and t.schdate=? and t.status=\'active\' group by t.schid;',
    ...
    getschbyids: (aid, fid, callback) => {
        pool.query(
            'select a.logo,t.schid,a.airlinename,f.flightnumber,t.source,t.destination,t.schdate,t.est_arrival_time,t.depature_time,t.firstclass,t.businessclass,t.economyclass,t.status from airline a join flight f on a.aid=f.aid join travelschedule t on t.fid=f.fid where a.aid=? and f.fid=? order by t.schdate asc',
            [
                aid,
                fid
            ],
    ...
    getschbyall: (aid, fid, date, callback) => {
        pool.query(
            'select a.logo,t.schid,a.airlinename,f.flightnumber,t.source,t.destination,t.schdate,t.est_arrival_time,t.depature_time,t.firstclass,t.businessclass,t.economyclass,t.status from airline a join flight f on a.aid=f.aid join travelschedule t on t.fid=f.fid where a.aid=? and f.fid=? and t.schdate=? order by t.schdate asc',
            [
                aid,
                fid,
                date
    ...
    getschedule: (schid, callback) => {
        pool.query(
            'select t.schid,t.fid,t.source,t.destination,t.est_arrival_time,t.depature_time,t.schdate,t.firstclass,t.businessclass,t.economyclass,f.flightnumber,a.airlinename,a.logo from airline a join flight f on f.aid=a.aid join travelschedule t on t.fid=f.fid  where schid=?',
            [
                schid
            ],
            (error, results, fields) => {
    ...
    updateflight: (data, callback) => {
        pool.query(
            'update flight set aid=?,flightnumber=?,focc=?,eocc=?,bocc=? where fid=?',
            [
                data.aid,
                data.flightnumber,
                data.focc,
    ...
    updateschedule: (data, callback) => {
        pool.query(
            'update travelschedule set fid=?,source=?,destination=?,est_arrival_time=?,depature_time=?,schdate=?,firstclass=?,businessclass=?,economyclass=? where schid=?',
            [
                data.fid,
                data.source,
                data.destination,
